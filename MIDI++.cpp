...
public:
    ...
    static std::pair<std::string, std::string> parsePair(const std::string& line) {
...
void loadConfig() {
    ...
    if (configFile.is_open()) {
        ...
        while (std::getline(configFile, line)) {
            line = JSONParser::trim(line);
            if (line.empty() || line == "{" || line == "}") continue;
            auto [key, value] = JSONParser::parsePair(line);
            if (key.empty()) continue;
            else if (key == "MIN_VOLUME") config.MIN_VOLUME = std::stoi(value);
            else if (key == "VOLUME_STEP") config.VOLUME_STEP = std::stoi(value);
            else if (key == "INITIAL_VOLUME") config.INITIAL_VOLUME = std::stoi(value);
            else if (key == "ADJUSTMENT_INTERVAL_MS") config.ADJUSTMENT_INTERVAL_MS = std::stoi(value);
            else if (key == "DEFAULT_MAX_VOLUME") config.DEFAULT_MAX_VOLUME = std::stoi(value);
            else if (key == "SUSTAIN_CUTOFF") config.SUSTAIN_CUTOFF = std::stoi(value);

        }
    }
    else {
        config = { 10, 10, 100, 50, 100, 64 };
    }
}
...
WORD getSpaceScanCode() {
    return static_cast<WORD>(MapVirtualKey(VK_SPACE, MAPVK_VK_TO_VSC));
}
...
private:
    ...
    bool readChunk(char* buffer, size_t size) {
        if (!file.read(buffer, size)) return false;
        return true;
    }

    bool validateTrackLength(std::streampos trackEnd) {
        if (file.tellg() > trackEnd) return false;
        return true;
    }

    bool validateEventLength(uint32_t length, std::streampos trackEnd) {
        if (file.tellg() + std::streampos(length) > trackEnd) return false;
        return true;
    }
...
public:
    ...
    MidiFile parse(const std::string& filename) {
        reset();
        ...
        MidiFile midiFile;
        char headerChunk[4];
        if (!readChunk(headerChunk, 4) || std::string(headerChunk, 4) != "MThd") throw std::runtime_error("Invalid MIDI file: Missing MThd");
        uint32_t headerLength;
        if (!readInt32(headerLength) || headerLength != 6) throw std::runtime_error("Invalid MIDI header length");
        if (!readInt16(midiFile.format) || !readInt16(midiFile.numTracks) || !readInt16(midiFile.division)) throw std::runtime_error("Error reading MIDI header fields");
        if (midiFile.format > 2) throw std::runtime_error("Unsupported MIDI format: " + std::to_string(midiFile.format));

        for (int i = 0; i < midiFile.numTracks; ++i) {
            char trackChunk[4];
            if (!readChunk(trackChunk, 4) || std::string(trackChunk, 4) != "MTrk") throw std::runtime_error("Invalid MIDI file: Missing MTrk");
            ...
            while (file.tellg() < trackEnd) {
                ...
                if ((status & 0xF0) == 0x80 || (status & 0xF0) == 0x90 || (status & 0xF0) == 0xA0 || (status & 0xF0) == 0xB0 || (status & 0xF0) == 0xE0) {
                    if (!file.read(reinterpret_cast<char*>(&event.data1), 1) || !file.read(reinterpret_cast<char*>(&event.data2), 1)) break;
                    event.data1 = (event.data1 > 127) ? 127 : event.data1;
                    event.data2 = (event.data2 > 127) ? 127 : event.data2;
                ...
                if (!validateTrackLength(trackEnd)) {
                    std::cout << "This midi file is potentially corrupted, only valid data were loaded." << std::endl;
                    break;
                }
            }
